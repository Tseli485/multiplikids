...
(function () {
  ...
  function listen(langCode, opts) {
    ...
    // Résultats INTERMÉDIAIRES activés : un mot très court (« six », « εννιά »)
    // dit vite par un enfant met parfois plusieurs secondes à être « finalisé »
    // par Chrome (voire jamais, en mode continu) → on exploite aussi les
    // hypothèses provisoires, qui apparaissent en ~100-300ms, bien plus fiable
    // pour un chiffre isolé que d'attendre la finalisation.
    ...
    rec.onresult = function (e) {
      ...
      if (alts.length && opts.onResult) opts.onResult(alts, isFinal);
    };
