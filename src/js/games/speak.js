...
(function () {
  ...
  const NET_MAX = 5;        // erreurs réseau d'affilée (avec répit) avant message « vérifie Internet »

  // Panneau de diagnostic MASQUÉ par défaut (l'enfant ne doit jamais le voir).
  // Pour le réactiver en cas de souci futur, dans la console du navigateur :
  //   localStorage.setItem('multiplikids_debug', '1')
  function debugOn() { try { return localStorage.getItem('multiplikids_debug') === '1'; } catch (e) { return false; } }
  ...
  class Speak {
    ...
    next() {
      ...
      this.host.innerHTML =
        '<div class="game-stage screen-enter">' +
          ...
          (debugOn() ? '<pre id="sp-debug" style="margin-top:14px;padding:10px;background:#00000055;border-radius:8px;font-size:.7rem;line-height:1.4;color:#9fb4c7;white-space:pre-wrap;word-break:break-word;max-height:180px;overflow:auto"></pre>' : '') +
        '</div>';
    ...
    onErr(err) {
      ...
      if (err === 'network') {
        ...
        // reconnaissance vocale de Google. Souvent TRANSITOIRE (petit à-coup de
        // connexion lors d'une réouverture rapprochée du micro) — on laisse un
        // court répit avant de retenter, pour ne pas cumuler les échecs à tort.
        if (this.netErr >= NET_MAX) { this.hardStop('network'); return; }
        this.later(() => { if (this.active && !this.speaking) this.openMic(true); }, 1200);
        return;
      }
